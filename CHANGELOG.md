...
### Added
- **Configurable mobile notification priority (`mobile_notify.priority`).** ntfy priority was derived solely from the per-event color map, so on iOS the lower tiers (`default`/`low`) routinely arrived silently: the banner showed but no sound played. A new optional `mobile_notify.priority` config key (and a `peon mobile ntfy <topic> --priority=` flag) sets an explicit priority that applies to every event, overriding the per-event default. Accepts ntfy priority names (`max`/`urgent`, `high`, `default`, `low`, `min`) or numbers `1` to `5`; invalid values are ignored so behavior stays predictable, and the same value maps through to Pushover priorities. Backward compatible: unset keeps the original behavior. Documented in `README.md`, `README_zh.md`, `README_ja.md`, and `docs/public/llms.txt`. PR #535, thanks @berkbtasdemir.
- **Codex stable hooks: full event-set support with Windows parity.** Codex now ships a stable hook event set (`SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, `Stop`) delivered as JSON on stdin. `adapters/codex.sh` and `adapters/codex.ps1` map those to CESP while preserving the legacy `notify` argv callback as a non-breaking fallback. The headline behavior: a failed stable-hook `PostToolUse` now sounds `task.error` instead of being silently swallowed, with failure detected from a nested `tool_response` (error flags or non-zero exit code), a top-level exit code, or `success == false`. `PreToolUse` and successful `PostToolUse` stay silent to avoid per-tool chatter, and `codex.ps1` reaches functional parity with the bash adapter (including a stdin-read guard so the legacy notify path never blocks). PR #532, thanks @muunkky. Addresses #513.

### Fixed
- **Subagent sounds suppressed directly via the `agent_id` hook field.** When Claude Code's Task tool dispatched sub-agents, a sub-agent's failed Bash command played `task.error` and permission prompts played `input.required` even with `suppress_subagent_complete` enabled, because the `PostToolUseFailure` and permission paths never checked sub-agent context. Events fired from inside a sub-agent are now identified directly by the `agent_id` field Claude Code adds to their hook payloads and short-circuited to silence when `suppress_subagent_complete` is on, with no session-tracking or timing window needed (so parallel sub-agents cannot race the pending pack slot). `SubagentStart`/`SubagentStop` are excluded so pack inheritance and the flag-gated completion sound keep working. PR #537, thanks @josejimenezjr0. Closes #536.
- **macOS overlay banners survive hook exit.** The backgrounded `osascript` overlay launch shared the hook's process group, so when Claude Code reaped the hook the overlay was signalled before it could draw, and unfocused-terminal banners stopped appearing (notably under tmux). The overlay launch now uses `nohup` so it detaches and survives. PR #538, thanks @lestopher.
...
### Fixed
- **Copilot CLI tool failures now sound `task.error`.** `peon.sh`'s `PostToolUseFailure` handler gated the error sound on `tool_name == 'Bash'`, the exact name Claude Code uses, so Copilot CLI failures (which arrive with lowercase `bash`, or `unknown` for the generic `errorOccurred` event) stayed silent. Copilot-sourced failures that carry an error message now route to `task.error` regardless of tool casing, while Claude Code's Bash-only gating (which avoids noise from its many non-Bash tool failures) is unchanged.
- **GitHub Copilot CLI camelCase payload leak (`permissionRequest`).** Even when registered with the PascalCase event key (which is documented to produce the VS Code-compatible snake_case payload), Copilot CLI 1.0.48-1's `permissionRequest` event delivers a camelCase payload (`hookName`, `sessionId`, `toolName`, `toolInput`). Result: `peon.sh` / `peon.ps1` saw `hook_event_name=null` and silently exited, so every Copilot CLI permission popup went unannounced. Both runtimes now defensively normalize camelCase fields (`hookName` → `hook_event_name`, `sessionId` → `session_id`, `toolName` → `tool_name`, `toolInput` → `tool_input`, plus 9 more) to their snake_case counterparts before event routing, and the camelCase event-name map gains Copilot CLI fallbacks (`permissionRequest`, `notification`, `agentStop`, `userPromptSubmitted`, `postToolUseFailure`). Verified end-to-end against a real captured Copilot CLI payload. Pester regressions in `tests/install-ps1-hook-rewrite.Tests.ps1`.
...
- **`peon pause` now silences the relay daemon too.** The relay's `/play` handler never checked the `.paused` flag that `peon pause` writes, so remote sessions (SSH, devcontainer, Codespaces) kept playing sounds after the user paused. The handler now honors `PEON_DIR/.paused` and acknowledges with `OK: paused` instead of playing. Regression test in `tests/relay.bats`. Closes #521. Thanks @dgunay.
- **Hooks resolve `PEON_DIR` the same way `peon.sh` does.** `hook-handle-use.sh` and `hook-handle-rename.sh` hardcoded `${CLAUDE_CONFIG_DIR:-$HOME/.claude}/hooks/peon-ping` while `peon.sh` picks the candidate location that actually contains `packs/`. On install layouts where those disagree (notably Nix home-manager, where packs live under `~/.openpeon`), the hooks wrote `.state.json` and config to a path `peon.sh` never read, so `/peon-ping-rename` and `/peon-ping-use` silently lost their effect on the next event. The hooks now walk the same `packs/`-anchored fallback chain as `peon.sh`. PR #523. Thanks @orasimus.
- **Nix home-manager: Claude Code slash commands and `/peon-ping-rename` now work.** The flake installPhase did not ship `hook-handle-rename.sh`, the hook scripts could not find `python3` on the PATH Claude Code passes to `UserPromptSubmit` hooks, and the home-manager module did not install the skill `SKILL.md` files, so every `/peon-ping-*` command failed with `Unknown command`. The flake now ships and wraps the rename hook and the home-manager module installs all five skills. PR #522. Thanks @orasimus.
...
### Added
- **oh-my-pi (omp) adapter.** New TypeScript extension at `adapters/omp/peon-ping.ts` plus shell installer at `adapters/omp.sh` route omp's `ExtensionAPI` lifecycle events (`session_start`, `turn_start`, `turn_end`, `tool_result` with `isError`, `auto_compaction_start`, `session_shutdown`) through `peon.sh` so omp users get every peon-ping feature: packs, desktop notifications, trainer, mobile push, SSH/devcontainer relay, tab titles. `peon.sh` recognizes `source: "omp"` (and `omp-…` session-id-prefix fallback) and renders as `oh-my-pi` in notifications. Install with `bash adapters/omp.sh`. PR #514. Thanks @dmarsh-gusto.
- **Antigravity Python watcher (`adapters/antigravity-py.sh`).** Headless / macOS LaunchAgent variant of the existing Antigravity adapter that uses a Python `watchdog`-based filesystem watcher with a 25s idle threshold to detect task completion without false-triggering during tool-call pauses. The companion `adapters/antigravity-watcher.py` is a pure event emitter that outputs CESP JSON (`SessionStart`, `UserPromptSubmit`, `Stop`) to stdout, and the shell wrapper pipes those through `peon.sh` so volume, pack rotation, notifications, and mobile push all work. `--install` registers a macOS LaunchAgent at `~/Library/LaunchAgents/com.peonping.antigravity-py-adapter.plist`; Linux falls back to `nohup` + pidfile. Requires `pip3 install watchdog`. PR #510. Thanks @gamedaysuits. Closes #506.
- **Rovo Dev CLI: full Rovo event coverage and config backfill on update.** `adapters/rovodev.sh` now reads Rovo Dev's stdin JSON payloads and handles `on_user_prompt`, `on_tool_start`, `on_tool_end`, `on_session_start`, `on_session_end` in addition to the original `on_complete`, `on_error`, `on_tool_permission`. When `on_tool_permission` fires without a `tool_input` field the permission notification body is filled from the surrounding context instead of showing `bash`. `install.sh --rovodev-only` no longer early-skips on any `rovodev.sh` reference; it walks the existing eventHooks config and inserts only the missing event entries, so existing installs pick up the new events on the next update. PR #512. Thanks @RandyTheAtlassian.

### Fixed
- **`install.ps1`: coalesce missing `matcher` to `""` when preserving sibling third-party hooks.** Accessing a missing `.matcher` property on a `PSCustomObject` yields `$null`, which `ConvertTo-Json` then serialized as literal JSON `null`. Claude Code's settings validator rejects null matchers with `"matcher: Expected string, but received null"`, breaking installs whose `~/.claude/settings.json` already contained sibling hook entries without an explicit matcher. The installer now coalesces missing matchers to `""` (the documented "match all" form) before serializing. Pester regression in `tests/install-ps1-hook-rewrite.Tests.ps1`. PR #515. Thanks @CrazyCoder.
